...
public class InstitutionController implements Serializable {
    ...
    public List<Institution> getSelectedBanks() {
        String sql;
        HashMap m = new HashMap();
        m.put("type", InstitutionType.Bank);
        if (selectText.trim().equals("")) {
            ...
            sql="select c from Institution c where c.retired=false and upper(c.name) like '%" + getSelectText().toUpperCase() + "%' and c.institutionType=:type order by c.name";
            selectedItems = getFacade().findBySQL(sql,m);
        }
...
//    }
    ...
    public void saveSelectedBank() {
        System.out.println("getCurrent().getInstitutionType() = " + getCurrent().getInstitutionType());
        getCurrent().setInstitutionType(InstitutionType.Bank);
        if (getCurrent().getId() != null && getCurrent().getId() > 0) {
            getFacade().edit(getCurrent());
            UtilityController.addSuccessMessage("savedOldSuccessfully");
        } else {
