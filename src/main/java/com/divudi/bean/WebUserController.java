...
public class WebUserController implements Serializable {
    ...
    public void updateUser(WebUser wu) {
        ...
        getFacade().edit(wu);
        UtilityController.addSuccessMessage("User Updated");
        fillUsers();
    }

    public void fillUsers() {
        webUsers = new ArrayList<>();
        webUsers = getSelectedItems();
    }

    public List<WebUser> completeUser(String qry) {
        List<WebUser> a = null;
        if (qry != null) {
            a = getFacade().findBySQL("select c from WebUser c where c.retired=false and "
                    + " (upper(c.webUserPerson.name) like '%" + qry.toUpperCase() + "%' or upper(c.code) like '%" + qry.toUpperCase() + "%')"
