...
public class BillNumberBean {
    ...
    public String summeryBillNumberGenerator(WebUser webUser, Summery s) {

        String sql;
        Map m = new HashMap();

        sql = " select count(bi) from BillItem bi where "
                + " bi.item.institution=:ins "
                + " and bi.item.department=:dep "
                + " and type(bi.item)=:class "
                + " order by bi.createdAt ";

        m.put("ins", s.getInstitution());
        m.put("dep", s.getDepartment());
        m.put("class", Summery.class);

        int result = 0;
        Long i = getBillFacade().findAggregateLong(sql, m, TemporalType.DATE);
        ...
        String st = s.getInstitution().getInstitutionCode() + s.getDepartment().getDepartmentCode() + df.format(result);
