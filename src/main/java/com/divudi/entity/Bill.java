...
//    number because supplier(Agent who come to get the order) can be vari
    ...
    @OneToOne
    private CashTransaction referanceCashTransaction;

    @Transient
    private boolean ok;
    @Transient
    double settleItemTotal;
    ...
    public boolean checkActiveForwardReference() {
        for (Bill b : getForwardReferenceBills()) {
            ...
            System.out.println("b.getCreater() = " + b.getCreater());
    ...
    public boolean checKBillItemStatus() {
        ...
        if (billItems != null) {
            ...
            for (BillItem bi : billItems) {
                if (bi.getItem() instanceof Settle) {
                    items.add(bi);
                    settleItemTotal += bi.getNetValue();
                }
            }
        }
    ...
    public List<BillItem> getBillItemsSettleOnly() {
        List<BillItem> items = new ArrayList<>();
        if (billItems != null) {
            for (BillItem bi : billItems) {
                ...
                if (bi.getItem() instanceof Settle) {
                    if (!bi.getItem().getName().equals("Cash Out")) {
                        items.add(bi);
                    }
                }
            }
...
//            //System.err.println("3 " + b.getBillType());
    ...
    public CashTransaction getReferanceCashTransaction() {
        return referanceCashTransaction;
    }

    public void setReferanceCashTransaction(CashTransaction referanceCashTransaction) {
        this.referanceCashTransaction = referanceCashTransaction;
    }
    ...
    public double getSettleItemTotal() {
        if (billItems != null) {
            settleItemTotal = 0.0;
            for (BillItem bi : billItems) {
                if (bi.getItem() instanceof Settle) {
        ...
        return settleItemTotal;
    }

    public void setSettleItemTotal(double settleItemTotal) {
        this.settleItemTotal = settleItemTotal;
    }
