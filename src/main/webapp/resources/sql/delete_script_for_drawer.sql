...
delete from cashtransactionhistory where `ID`>0;
...
UPDATE drawer set`CHEQUEBALLANCE`=0,`CREDITCARDBALLANCE`=0,`RUNNINGBALLANCE`=0,`SLIPBALLANCE`=0,`CREDITBALLANCE`=0,
`IOUBALLANCE`=0,`shortBallance`=0,`CASHIEREXCESSBALLANCE`=0,`CASHIERSHORTBALLANCE`=0 WHERE `ID`>0;
UPDATE department set `BALANCE`=0 WHERE `ID`>0;
